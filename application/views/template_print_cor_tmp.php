...
$sched = array_merge($sched,$sun);            
    ...
    <tr>      
      ...
      <td>
            ...
            <?php 
              if ( ! (empty($subject['second_time']) || empty($subject['second_day'])) )
              {
                echo $subject['second_time'] . ' ' . $subject['second_day'];
              }
            ?>
...
<?php
  // $SubTotal = 0.00;
  // $TuitionFee = 10000.00;
  // $Exemption = $AssDetails3 ? '<code>'.$AssDetails3['ScholarshipDesc'] .'</code>': '';
  // $CityHall = ($TuitionFee - $AssDetails4[0]['Amount']); //$_SESSION['student_info']['IsMakatiResident'] == 1 ? 9000.00 : 7000.00;
  // $Discount = $AssDetails3 ? $AssDetails4[0]['Amount'] * $AssDetails3['DiscountPercentage'] : 0;
  // $TokenFee = $TuitionFee - $CityHall - $Discount;

  // $NstpFee = 0.00;
  // $NstpTotal = 0.00;
  // $NstpDesc = '';
  // foreach($AssDetailsNstp as $fees) : 
  //     $NstpDesc = $fees['FeeDesc'];
  //     $NstpFee = $fees['Amount']; 
  //     $NstpTotal = $NstpTotal + $NstpFee;
  // endforeach; 

  // $LabFee = 0.00;
  // $LabTotal = 0.00;
  // $LabDesc = '';
  // foreach($AssDetailsLabFee as $fees) : 
  //     $LabDesc = $fees['FeeDesc'];
  //     $LabFee = $fees['Amount']; 
  //     $LabTotal = $LabTotal + $LabFee;
  // endforeach; 

  #if($_SESSION['student_curriculum']['CollegeId'] != "6") : # Only Regular Student 
...
?>
      ...
      <tbody class="table-condensed">
          ...
          <?php if ( ! empty($nstp_payment)): ?>
            ...
            <tr>
              ...
              <td><?php echo $nstp_payment['ornum'] ?></td>
            </tr>
            <tr>
              <th style="text-align:right; padding-right:45px;font-size:16px">Amount</th>
              <th style="font-size:16px"><?php echo nf($nstp_payment['Amount']) ?></th>
            </tr>
      ...
      <tr>
        ...
        <td>
          <?php
            if ($TokenFee > 0) 
            {
              if (isset($_SESSION['payment_mode']['PayMode'])) 
              {
                if ($_SESSION['payment_mode']['PayMode'] == "PROMISSORY") 
                  echo "PROMISSORY NOTE";
                else
                {
                  if (($TokenFee - $_SESSION['token_fee_payment']['Amount']) == 0)
                  echo "CASH";
                  else
                  echo "INSTALLMENT";
                }
        ...
        <td class="lead">
          <?php 
            if ($TokenFee > 0)
            {
              ...
              {
                if ($_SESSION['payment_mode']['PayMode'] != "PROMISSORY")
                  echo $_SESSION['token_fee_payment']['receipt'][0]['ornum'];
              }
              else
              echo $_SESSION['token_fee_payment']['receipt'][0]['ornum'];
            }
        ...
        <td class="lead" style="vertical-align:bottom;">
          ...
          <?php 
            if ($TokenFee > 0)
            {
              ...
              {
                if ($_SESSION['payment_mode']['PayMode'] != "PROMISSORY")
                  echo date('m/d/Y', strtotime($_SESSION['token_fee_payment']['receipt'][0]['Date']));
              }
              else
              echo date('m/d/Y', strtotime($_SESSION['token_fee_payment']['receipt'][0]['Date']));
            }
              ...
              <span class="pull-right">
                <?php 
                  ...
                  {                    
                    ...
                    {
                      if ($_SESSION['payment_mode']['PayMode'] != "PROMISSORY")
                        echo number_format($_SESSION['token_fee_payment']['receipt'][0]['Amount'],2);
                    }
                    else
                    echo number_format($_SESSION['token_fee_payment']['receipt'][0]['Amount'],2);
                  }
              ...
              <span class="pull-right">
                <?php 
                  ...
                  {                    
                    ...
                    {
                      if ($_SESSION['payment_mode']['PayMode'] != "PROMISSORY")
                        echo number_format(($TokenFee - $_SESSION['token_fee_payment']['receipt'][0]['Amount']),2);
                    }
                    else
                    echo number_format(($TokenFee - $_SESSION['token_fee_payment']['receipt'][0]['Amount']),2);
                  }
        ...
        <td style="vertical-align:bottom;">
          <?php 
            ...
            {              
              ...
              {
                if ($_SESSION['payment_mode']['PayMode'] != "PROMISSORY")
                  echo $_SESSION['token_fee_payment']['receipt'][0]['User'];
              }
              else
              echo $_SESSION['token_fee_payment']['receipt'][0]['User'];
            }
  ...
  <!--/. assessment ./-->
   <?php dump($_SESSION['token_fee_payment']); ?>
</div>
